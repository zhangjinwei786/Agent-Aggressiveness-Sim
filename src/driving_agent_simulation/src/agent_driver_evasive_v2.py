		...
		if len(neighborPosesList) == 0:
			...
			for p in neighborPosesList:
				...
				if detect_x_range[0]<=(p[0]-agent_x_pos)<=detect_x_range[1] and detect_y_range[0]<=(p[1]-agent_y_pos)<=detect_y_range[1]:
					...
					# print("obstacle in y gradient is: " + str(float((P * dirY / sqrt(dirX**2 + dirY**2)).subs({x:agent_x_pos, y:agent_y_pos}))))
...
def main(argv):
	...
	while not rospy.is_shutdown():
		...
		if node.godSwitch:
			node.potentialField()
			node.steeringCtrl(12, 0.18, 7, 0.25)	# (high_speed, highLimit, low_speed, lowLimit)
			node.speedController(7, 12, 15) 		# (low-speed, high-speed, threshold of gradient magnitude)
